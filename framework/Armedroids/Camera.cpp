...
namespace CompEngine
{
	...
	Matrix Camera::GetViewMatrix(GameObject* owner)
	{
		...
		{
			cout << "Camera" << ID << " doesn't have Transform3D component." << endl;
	...
	void Camera::FixedUpdate(GameObject* owner)
	{
		...
		{
			Transform3D* transform3D = ((Transform3D*)owner->GetComponent("Transform3D"));

			Vec3 Position(0, 0, 0);

			if (transform3D != nullptr)
				Position = transform3D->GetPosition();
			else
				cout << "Camera" << ID << " doesn't have Transform3D component." << endl;

			D3DXVec3Normalize(&viewVector, &(targetPosition - transform3D->GetPosition()));//->GetWorldPositon()));
			...
			DeviceMgr->GetDevice()->SetTransform(D3DTS_PROJECTION, &projectionMatrix);
		}
	}
	...
	void Camera::RotateLocalX(float angle)
	{
		...
		D3DXVec3Normalize(&viewVector, &(targetPosition - Tr->GetWorldPosition()));
		D3DXVec3Cross(&crossVector, &cameraUp, &viewVector);
	}
